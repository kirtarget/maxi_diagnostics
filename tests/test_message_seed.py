...
@pytest.mark.asyncio
async def test_seed_replaces_only_the_previous_default_text():
    ...
    assert "WHEN message_templates.text=$4 THEN EXCLUDED.text" in query
    welcome = next(row for row in rows if row[0] == "WELCOME")
    assert welcome == (
        "WELCOME",
