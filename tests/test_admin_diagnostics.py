...
def test_delete_user_requires_positive_id_json_confirmation_and_delete_method(monkeypatch):
    ...
    client = make_client(monkeypatch)
    router.repository.delete_diagnostic_user.return_value = {"notifications": 2, "attempts": 1, "engagements": 1}
    ...
    assert response.status_code == 200
    assert response.json() == {"ok": True, "deleted": {"notifications": 2, "attempts": 1, "engagements": 1}}
...
async def test_delete_repository_is_atomic_parameterized_and_diagnostic_only(monkeypatch):
    ...
    assert result == {"notifications": 2, "attempts": 1, "engagements": 1}
