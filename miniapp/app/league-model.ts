...
export function formatLeagueDates(start: string, end: string): string {
  const startDate = parseCalendarDate(start);
  const endDate = parseCalendarDate(end);
  if (!startDate || !endDate) return `${start} · ${end}`;
  const format = new Intl.DateTimeFormat("ru-RU", { day: "numeric", month: "long", timeZone: "UTC" });
  const startMonth = startDate.getUTCMonth();
  const endMonth = endDate.getUTCMonth();
  const sameMonth = startMonth === endMonth;
  if (sameMonth) {
    const month = format.formatToParts(startDate).find((part) => part.type === "month")?.value ?? "";
