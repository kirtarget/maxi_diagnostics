...
import { loadBootstrap, loadDailyPlan, loadWeeklyLeague, recordOfferEvent, startOnboarding } from "./api";
...
export function useBootstrap(setScreen: (screen: Screen) => void): BootstrapSession {
  ...
  const [planState, setPlanState] = useState<PlanScreenState>({ kind: "loading" });
  ...
  const openPlan = useCallback(async () => {
    ...
    setScreen("plan");
    try {
      setPlanState({ kind: "ready", data: await loadDailyPlan(initData.current, sessionScope) });
    } catch {
      setPlanState({ kind: "error", message: "Не удалось загрузить план. Повтори попытку." });
    }
  }, [sessionScope, setScreen]);
