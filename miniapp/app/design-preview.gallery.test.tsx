...
import type { Brand, PublicDiagnostic, Question, ServerResult } from "./types";

const OUT_DIR = process.env.DESIGN_PREVIEW_DIR ?? "";

const labels = brand.interface as unknown as Brand["interface"];
const links = { website: "#", support: "#", privacy: "#", offers: [{ id: "o1", label: "Интенсив по профильной математике — старт 1 сентября", button: "Узнать", url: "#" }] };
...
const diagnostics: PublicDiagnostic[] = [
  { id: "math", content_version: "v1", exam: "ОГЭ", subject: "Математика", mark: "М", quick_count: 10, questions: Array.from({ length: 24 }, (_, i) => q(String(i + 1))) },
  { id: "rus", content_version: "v1", exam: "ОГЭ", subject: "Русский язык", mark: "Р", quick_count: 10, questions: Array.from({ length: 20 }, (_, i) => q(`r${i + 1}`)) },
  { id: "phys", content_version: "v1", exam: "ОГЭ", subject: "Физика", mark: "Ф", quick_count: 10, questions: Array.from({ length: 24 }, (_, i) => q(`p${i + 1}`)) },
  { id: "chem", content_version: "v1", exam: "ЕГЭ", subject: "Химия", mark: "Х", quick_count: 10, questions: Array.from({ length: 22 }, (_, i) => q(`c${i + 1}`)) },
];
...
const screens: Array<[string, string]> = [
  ...
  ["trainer-feedback", renderToStaticMarkup(<TrainerScreen state={{
    ...
    draftAnswer: "b",
    submittedAnswer: "b",
    answerResult: { is_correct: true, correct_answer: "18", explanation: "Сначала скобки: 4 + 2 = 6, затем умножение: 3 · 6 = 18.", xp_delta: 15, lives_remaining: 4, revision: 2 },
    finishResult: null,
    error: null,
    retryPhase: "answering",
  } as never} dispatch={noop} />)],
