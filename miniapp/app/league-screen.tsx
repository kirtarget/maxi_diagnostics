...
import { formatLeagueDates, type LeagueResponse, type LeagueRow, type LeagueScreenState } from "./league-model";
...
function LeagueReady({ data, onTrain }: { data: LeagueResponse; onTrain?: () => void }) {
  const dates = formatLeagueDates(data.week_start, data.week_end);
  if (data.status === "forming") {
    return (
      <section className="league-screen" aria-labelledby="league-title">
        ...
        <p className="league-dates">{dates}</p>
        {onTrain && <button className="primary-button" onClick={onTrain} type="button">Начать тренировку</button>}
      </section>
    );
  }
...
export function LeagueScreen({ state, onRetry, onHome, onTrain }: { state: LeagueScreenState; onRetry?: () => void; onHome?: () => void; onTrain?: () => void }) {
  ...
  return <>{onHome && <button className="text-back league-back" onClick={onHome} type="button">На главную</button>}<LeagueReady data={state.data} onTrain={onTrain} /></>;
}
