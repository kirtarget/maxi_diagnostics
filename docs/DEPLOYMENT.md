...
- A school-controlled domain such as `app.example.com`.
...
Create an `A`/`AAAA` DNS record for the chosen host and wait until it resolves to the
server. Provision an HTTPS certificate with the host's normal ACME client. TLS
provisioning is intentionally provider-neutral; use the school infrastructure
policy and enable renewal monitoring.

Copy `deploy/nginx/diagnostic.conf.example` to the server Nginx configuration. Replace
every `app.example.com` and certificate path with the school host, validate with
`nginx -t`, then reload Nginx. The example serves one fixed host and same-origin routes:

- `/` goes to the Mini App at `127.0.0.1:13000`;
...
IMAGE_NAMESPACE=example-school-diagnostic
...
MINIAPP_ORIGIN=https://app.example.com
...
diagnostic content versions. Rotate it only as a planned security operation. Start
...
curl --fail http://127.0.0.1:13000/
```

Then check `https://app.example.com/healthz` through Nginx.
