...
Каталоги пополняются и другими импортами. Вопросы с `source.provider` вне кампании
(например `maximum_editorial` из SharePoint) считаются внешними дополнениями: они
обязаны идти строго после baseline и слотов кампании, иначе проверка падает с
`campaign.catalog.external_not_appended`. SHA-256 и количества считаются по файлу
без этих вопросов, поэтому пинованные хеши manifest остаются валидными. Их общее
число выводится в отчёте как `external_additions`.

`audit_editorial_readiness.py` считает такие вопросы отдельно: `summary.by_provider`
показывает строку на каждый provider, включая `maximum_editorial`, а каждый item
несёт поле `provider`.
